...
| `unsupported TACO version '2.0.0' in <source>; expected 3.0.0` | A v2 dataset; rebuild it |
...
- Extension descriptors are never written to `COLLECTION.json`. The lower-level
  `Contract.from_dict` has a compatibility branch for `taco:extensions`, but
  `Collection.from_dict` rejects that reserved key first. Do not author it; see
  extensions.md.
