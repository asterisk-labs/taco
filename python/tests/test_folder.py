...
def test_folder_append_restores_metadata_when_publish_fails(
    tmp_path: Path, collection, make_sample, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    def fail_on_sample(source: Path, target: Path) -> None:
        if source.name == "sample.parquet" and target.parent.name == "METADATA":
            raise OSError("publish failed")
        native_move(source, target)

    monkeypatch.setattr(publish_module, "_move_without_replacing", fail_on_sample)
